...
const RenderFileUploadStatus = (props:any) => {
  ...
  return (
    <Box sx={{ width: '100%', border: 2, borderColor:'lightgray', borderRadius: 3 ,padding:1, display: 'flex', maxWidth: '45%', minWidth: 300 }}>
        <MdFilePresent size={25} style={{marginRight: 2, minWidth: 25}}/>
        <Box sx={{ mx: 'auto', width: '85%'}}>
            <Typography sx={{fontSize: 15, fontWeight: 'bold', wordWrap:'break-word', width:'100%'}}>CirculationStatusMapping.csv</Typography>
            <Typography sx={{fontSize: 13, wordWrap:'break-word', width:'100%'}}>15KB</Typography>
            <BorderLinearProgress variant="determinate" value={progress} aria-label='file-upload-progress-bar' />
        </Box>
        <Box sx={{display: 'flex', alignItems: 'center', wordWrap:'break-word'}}>
            <IconButton aria-label='close-file-upload-component' onClick={props.hideComponent}>
                <MdClose/> 
            </IconButton>
        </Box>
...
export default function FileUploadStatus() {
  ...
  return(
    <>
      ...
      { hidden == true ? null : <RenderFileUploadStatus hideComponent={() => HandleComponentToggle()}/>}
    </>
