...
files.forEach((filePath) => {
	...
	const hasGetStatic =
		content.includes("getStaticProps") || content.includes("getStaticPaths");
	...
	if (needsRouteInjection) {
		...
		if (!componentName) {
			match = content.match(
				/const\s+([A-Z][A-Za-z0-9_]+)\s*=\s*(?:async\s*)?\([^)]*\)\s*=>/,
			);
			if (match) componentName = match[1];
		}
		...
		if (componentName) {
			...
			console.log(
				`⚠️ Route Injection Skipped (Couldn't find component name): ${filePath}`,
			);
		}
	...
	if (hasSession) {
		if (!content.includes("useAuth")) {
			content = content.replace(
				/import\s+{\s*useSession\s*}\s*from\s*['"]next-auth\/react['"];?/g,
				'import { useAuth } from "react-oidc-context";',
			);
		} else {
			content = content.replace(
				/import\s+{\s*useSession\s*}\s*from\s*['"]next-auth\/react['"];?\n?/g,
				"",
			);
		}

		const sessionRegex =
			/const\s+(?:\{\s*[^}]+\s*\}|[a-zA-Z0-9_]+)\s*=\s*useSession\(\s*\{[\s\S]*?\}\s*\);|const\s+(?:\{\s*[^}]+\s*\}|[a-zA-Z0-9_]+)\s*=\s*useSession\(\);/g;
		content = content.replace(
			sessionRegex,
		...
		content = content.replace(
			/session\?\.profile\?\.roles\?\.some\(\(role:\s*string\)\s*=>\s*adminOrConsortiumAdmin\.includes\(role\),?\s*\)/g,
			"isAnAdmin",
		);
		modified = true;
	}
	...
	// ==========================================
	if (hasRouterQuery) {
		const routerParamsDestructureRegex =
			/const\s+\{\s*([a-zA-Z0-9_,\s]+)\s*\}\s*=\s*router\.query;/g;
		content = content.replace(
			routerParamsDestructureRegex,
			"const { $1 } = Route.useParams();",
		);

		const routerParamsDotRegex =
			/const\s+([a-zA-Z0-9_]+)\s*=\s*router\.query\.[a-zA-Z0-9_]+(?:\s*as\s*string)?;/g;
		content = content.replace(
			routerParamsDotRegex,
	...
	if (hasGetStatic) {
		content = content.replace(
			/export\s+async\s+function\s+getStatic(Props|Paths)[\s\S]*?^}/gm,
			"",
		);
		modified = true;
	}
	...
	if (hasOldQueryImports) {
		const queriesImportRegex =
			/import\s+\{([\s\S]*?)\}\s+from\s+['"](?:src\/)?queries\/queries['"];?/g;
	...
	if (hasApollo) {
		content = content.replace(
			/import\s+{\s*useQueryClient(,\s*)?useMutation(,\s*)?useQuery\s*}\s*from\s*['"]@apollo\/client.*['"];?/g,
			'import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";\nimport { useGraphQLClient } from "@/hooks/useGraphQLClient";',
		);
		content = content.replace(
			/import\s+{\s*useMutation(,\s*)?useQuery\s*}\s*from\s*['"]@apollo\/client.*['"];?/g,
			'import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";\nimport { useGraphQLClient } from "@/hooks/useGraphQLClient";',
		);
		modified = true;
	}
