...
describe("Agencies page", () => {
	beforeEach(() => {
		...
		cy.intercept("POST", "/graphql", { fixture: "agencies.json" }).as(
			"getAgencies",
		);
	});
	it("should render the agencies page with the correct data and UI elements.", () => {
		...
		cy.get("[data-id=dd51ff90-55ae-5225-be90-fa82c1f0f0ff]").should(
			"contain",
			"The Great Test University",
		);
	});
	...
	it("should search for an agency and return the expected result", () => {
		...
		cy.get("[data-id=d74636f3-3ab5-5325-922e-a46e20fe52e7]").should(
			"contain",
			"Benedictine College",
		);
	});
