...
describe("Patron requests page", () => {
	beforeEach(() => {
		...
		cy.intercept("POST", "/graphql", { fixture: "patronRequests.json" }).as(
			"loadRequests",
		);
	});
	it("should render the Patron requests page with the correct data", () => {
		...
		cy.get("[data-id=jb3b9a22-2ab3-4118-ae81-61932265777e]").should(
			"contain",
			"Another test request strikes again!",
		);
	});
	...
	it("should search for a given request by title, and display the expected result", () => {
		...
		cy.get("[data-id=zi3b9a22-2ab3-4118-ae81-61932265777e").should(
			"contain",
			"Return of the test request",
		);
	});
	afterEach(() => {
		cy.logout();
